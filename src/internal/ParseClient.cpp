...
const unsigned short SSL_PORT = 1337;
...
const char* ParseClient::getInstallationId() {
  if (!strlen(installationId)) {
    ...
    ParseResponse response = sendRequest("POST", "/installations", content, "");
    if (Serial && DEBUG) {
...
ParseResponse ParseClient::sendRequest(const String& httpVerb, const String& httpPat, const String& requestBody, const String& urlParams) {
  char buff[91] = {0};
  //client.stop();
  String httpPath="/parse"+httpPat;
  saveKeys();
